...
int dev_scan()
{
	DEBUG_MSG("scan for serial ports");
#ifdef _WIN32
	int i;
	/* test serial port devices (COM1 to COM9) */
	for (i = 1; i <= 9; i++)
	{
		char port_name[16];
		COMMCONFIG config;
		...
		h = CreateFileA(port_name, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, 0, 0);
		if (h == INVALID_HANDLE_VALUE)
		{
			/* port does not exist, next */
			continue;
		}
		if (GetCommConfig(h, &config, &size))
		{
			INFO_MSG("found serial port: %s, type: %d, %d", port_name, config.dwProviderSubType, PST_RS232);
		}
		else
		{
			ERROR_MSG("config read failed: %s", port_name);
		}
		CloseHandle(h);
	}
#endif
