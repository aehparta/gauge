...
struct devlib_process *devlib_process_create(struct device *source)
{
	int err = 0;
	struct devlib_process *process = NULL;

	/* check that primary device is the right type */
	IF_ERR(source->type != DEV_TYPE_IO_READ, -1, "primary process device must be of type DEV_TYPE_IO_READ");
...
int devlib_process_insert(struct devlib_process *process, int index, struct device *dev)
{
	int err = 0;
